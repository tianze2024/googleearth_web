...
const inputHandler = (obj: { flag: boolean, index: number }) => {
    const bools = JSON.parse(JSON.stringify(boolean3Ref.value))
    bools[obj.index] = obj.flag
    boolean3Ref.value = bools
}
const toUndefined = () => {
...
// const addFlag = () => {
//     boolean3Ref.value.push(true)
// }
// const deleteFlag = (index: number) => {
//     boolean3Ref.value.splice(index, 1)
// }
const cancel = () => {
    boolean3Ref.value = []
...
const ok = () => {
    const arr = JSON.parse(JSON.stringify(boolean3Ref.value)) as [boolean, boolean, boolean]
...
<template>
    ...
    <Window :show="viewsShow" :title="'boolean数组'" :okText="'确认'" :cancelText="'取消'" :onlyButton="false" :readonly="readonly"
        ...
        <div class="Positions">
            ...
            <div class="Positions_line" v-for="(_, index) in boolean3Ref" :key="index">
                <div class="booleansRef">
                    <span>{{ index }}</span>
                    <ToggleSwitch :booleanValue="boolean3Ref[index]" :index="index" @update:booleanValue="inputHandler">
                    </ToggleSwitch>
